...
/**
 * 根据移动api状态，转换为本平台状态
 * @param status 00-正常；01-单向停机; 02-停机；03-预销号; 04-销号；05-过户; 06-休眠；07-待激; 99-号码不存在
 * @param lifeCycle 00:正式期，01:测试期，02:沉默期，03:其他。
 * @returns {number}
 */
function parseCmStatus(status, lifeCycle){
    switch (status){
        case '00' : {
            switch(lifeCycle){
                case '00' : return 3;
...
/**
 * 根据电信公司api状态，转换为本平台状态
 * @param status
 */
function parseCtStatus(status){
    switch (status){
        case '1':{
            return 3;
        }
