  ...
  <div>
    <el-row>
      ...
      <el-col :span="12" v-if="media.type === 4">
        <el-button type="primary" class="media-edit-add" @click="showSuggestionForm(true)"><i class="el-icon-circle-plus"> 新增悬浮菜单</i></el-button>
      ...
      <el-empty v-if="(media.list.length === 0)" :description="media.emptyTip"></el-empty>
        <el-col v-else :span="media.rowNumber" v-for="item in media.list" :key="item._id">
          ...
          <el-card shadow="hover" :body-style="cardBody" class="media-card-panel">
            ...
            <div v-else-if="item.type === 3" class="image-ctn"  :style="'background-image:url('+item.thumbnail_url+');background-size:cover'">
<!--              <el-image class="image-ctn" :src="item.thumbnail_url" :fit="fit"></el-image>-->
            ...
            <div v-if="item.type !== 4">
              ...
              <div class="bottom clearfix">
                ...
                <el-button type="text" class="button" @click="onDeleteMedia(item._id)"><i class="el-icon-delete"> 删除</i></el-button>
              </div>
            </div>
            ...
            <div v-if="item.type === 4">
              <el-card>
                <div slot="header" class="clearfix">
                  <span>{{item.name}}</span>
                  <el-button class="btn-type-4" type="primary" size="mini" icon="el-icon-delete" @click="onDeleteMedia(item._id)">删除</el-button>
                  <el-button class="btn-type-4" type="primary" size="mini" icon="el-icon-edit" @click="onEditSuggestion(item)">编辑</el-button>
                </div>
...
<script>
  ...
  import suggestionForm from './mediaSuggestionForm'
  ...
  export default {
    ...
    methods: {
      ...
      showSuggestionForm(flag){
        this.$refs.suggestionForm.visible = flag;
      },
      async onEditSuggestion(media){
        this.showSuggestionForm(true);
        let menus = await getMenus(media._id);
        this.$refs.suggestionForm.setForm({
          id : media._id,
          name : media.name,
          type : 4,
          menus : menus
