...
export default {
  ...
  methods: {
    // 弹窗打开触发的方法
    openFun() {
      this.allMenu = [].concat(this.userMenus);
    ...
    handleSubmit(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          this.formData.pid = this.formData.pid.pop();
          this.$emit(this.dialogMenu.option, this.formData);
        } else {
  ...
  watch: {
    'formData.type': {
      handler: function() {
        if (this.formData.type == 1) {
          this.formData.redirect = 'noRedirect' //建议 必须是 ‘noRedirect’, 与面包屑有关
          this.formData.alwaysShow = true//true: 总是显示父级，false: 当只有一个子菜单的时候，不显示父级
